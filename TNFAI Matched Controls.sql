-------------------------------------------------------------------------------- 
-- Name: Autoimmune Disease and Cancer - Matched Controls
-- Author: NM\psilber1
-- Date: 11/5/2020
-- Task: RW-2436
-- IRB: STU00211830
-- Description: These are matched controls to patients that have TNFA-I drugs for
-- autoimmune disease. Controls have no autoimmune disase and no TNFA-I drugs.
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- Report Parameters. 
--------------------------------------------------------------------------------

--DECLARE @cohort_id AS INT = 4955;
...
SELECT DISTINCT p.ir_id
	, p.patient_key
	, cp.patient_study_id
	, COALESCE(pat.pat_mrn_id,p.west_mrn) AS NMHC_MRN
...
FROM dim.vw_patient_current p
	...
	JOIN FSM_Analytics.cohort.[cohorts] c 
		...
		AND c.[is_dltd_flg] = 0
	LEFT JOIN cr_xwalk.vw_patient cx
		ON cx.ir_id = p.ir_id
		AND cx.source_system_key = 1
	LEFT JOIN clarity.dbo.patient pat
		ON pat.pat_id = cx.source_system_id
;


SELECT ccm.control_ir_id
	, cp.patient_study_id
INTO #casecontrols
FROM FSM_EDW.reports_dm.rw2436_case_control_matching ccm
	JOIN FSM_Analytics.cohort.[cohort_patients] cp
		ON cp.source_ir_id = ccm.case_ir_id
		AND cp.[is_dltd_flg] = 0 
		AND cp.[cohort_id] = 4856 --TNFAI cases
	JOIN FSM_Analytics.cohort.[cohorts] c 
		ON c.[cohort_id] = cp.[cohort_id] 
		AND c.[is_dltd_flg] = 0
;
...
--This is from the case report, so can skip because a prescription for these drugs
--is part of the exclusion criteria
/*
--------------------------------------------------------------------------------
...
FROM (
	...
	FROM ( 
		...
		AND mo.order_start_date_key <= GETDATE()	--medication starts prior to today
		AND (mo.order_end_datetime IS NULL OR mo.order_end_datetime > GETDATE())	--medication ends after today or does not end
		) x
	) y
GROUP BY ir_id
;
...
SELECT ir_id
	...
	, MAX(CD) AS CD
	, MAX(UC) AS UC
	, MAX(PSORIASIS) AS PSORIASIS
	, MAX(LYNCH_SYNDROME) AS LYNCH_SYNDROME
	, MAX(GARDNERS_SYNDROME) AS GARDNERS_SYNDROME
	, MAX(LI_FRAUMENI_SYNDROME) AS LI_FRAUMENI_SYNDROME
	, MAX(HPV) AS HPV
	, MAX(EBV) AS EBV
	, MAX(HBV) AS HBV
	, MAX(HCV) AS HCV
	, MAX(H_PYLORI) AS H_PYLORI
	, MAX(History_of_abnormal_DRE) AS History_of_abnormal_DRE
	, MAX(Ankylosing_Spondylitis) AS Ankylosing_Spondylitis
	, MAX(hidradenitis_suppurativa) AS hidradenitis_suppurativa
	, MAX(uveitis) AS uveitis
	, MAX(psoriatic_arthritis) AS psoriatic_arthritis
	, MAX(juvenile_idiopathic_arthritis) AS juvenile_idiopathic_arthritis
	, MAX(spondyloarthritis) AS spondyloarthritis
	, MAX(behcets_disease) AS behcets_disease
INTO #comorb
FROM (
	SELECT c.ir_id
	...
	WHERE de.start_date_key > '1900-01-01'	--these are not reliable dates, so excluding them
	) x
GROUP BY ir_id
;
...
--------------------------------------------------------------------------------
-- Cancer by category each diagnosis
--------------------------------------------------------------------------------
SELECT ir_id
...
-- Compilation
--------------------------------------------------------------------------------
SELECT DISTINCT c.patient_study_id AS control_patient_id
	, cc.patient_study_id AS case_patient_id
	, c.NMHC_MRN
	, c.Cerner_MRN
	, c.Legacy_Epic_MRN
	, c.first_name
...
FROM #cohort c
	JOIN #casecontrols cc
		ON cc.control_ir_id = c.ir_id
